...
public class AGG02 extends TestHarness {
  public static void main(String args[]) {
    TestHarness.main(args, new AGG02());
  }
  public void testRA() {
    ...
    query_0.addAggregate(new AggregateNode.AggColumn("Average", new ExprTree.VarLeaf(null, "A"),AggregateNode.AType.AVG));
    query_0.setChild(child_1);
    TestHarness.testQuery(tables, getResults0(), query_0);
    System.out.println("Passed RA Test AGG02");
  }
  public void testSQL() {
    List<List<Datum[]>> expected = new ArrayList<List<Datum[]>>();
    expected.add(getResults0());
    TestHarness.testProgram(new File("test/AGG02.SQL"),
                            expected);
    System.out.println("Passed SQL Test AGG02");
  }
  ArrayList<Datum[]> getResults0() {
    ArrayList<Datum[]> ret = new ArrayList<Datum[]>();
    ret.add(new Datum[] {new Datum.Flt(3.2)}); 
    return ret;
  }
}
