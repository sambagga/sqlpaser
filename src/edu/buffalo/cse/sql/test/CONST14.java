...
public class CONST14 extends TestHarness {
  public static void main(String args[]) {
    TestHarness.main(args, new CONST14());
  }
  public void testRA() {
    ...
    query_0.addColumn(new ProjectionNode.Column("EXPR", new ExprTree(ExprTree.OpCode.MULT, new ExprTree(ExprTree.OpCode.ADD, new ExprTree.ConstLeaf(1), new ExprTree.ConstLeaf(1)), new ExprTree.ConstLeaf(2))));
    query_0.setChild(child_1);
    TestHarness.testQuery(tables, getResults0(), query_0);
    System.out.println("Passed RA Test CONST14");
  }
  public void testSQL() {
    List<List<Datum[]>> expected = new ArrayList<List<Datum[]>>();
    expected.add(getResults0());
    TestHarness.testProgram(new File("test/CONST14.SQL"),
                            expected);
    System.out.println("Passed SQL Test CONST14");
  }
