...
public class AGG11 extends TestHarness {
  public static void main(String args[]) {
    TestHarness.main(args, new AGG11());
  }
  public void testRA() {
    ...
    query_0.addAggregate(new AggregateNode.AggColumn("Sum", new ExprTree(ExprTree.OpCode.MULT, new ExprTree.VarLeaf(null, "A"), new ExprTree.VarLeaf(null, "C")),AggregateNode.AType.SUM));
    query_0.setChild(child_1);
    TestHarness.testQuery(tables, getResults0(), query_0);
    System.out.println("Passed RA Test AGG11");
  }
  public void testSQL() {
    List<List<Datum[]>> expected = new ArrayList<List<Datum[]>>();
    expected.add(getResults0());
    TestHarness.testProgram(new File("test/AGG11.SQL"),
                            expected);
    System.out.println("Passed SQL Test AGG11");
  }
  ArrayList<Datum[]> getResults0() {
    ArrayList<Datum[]> ret = new ArrayList<Datum[]>();
    ret.add(new Datum[] {new Datum.Int(156)}); 
    return ret;
  }
}
