...
public class CONST06 extends TestHarness {
  public static void main(String args[]) {
    TestHarness.main(args, new CONST06());
  }
  public void testRA() {
    ...
    query_0.addColumn(new ProjectionNode.Column("EXPR", new ExprTree.ConstLeaf(1.2E-4)));
    query_0.setChild(child_1);
    TestHarness.testQuery(tables, getResults0(), query_0);
    System.out.println("Passed RA Test CONST06");
  }
  public void testSQL() {
    List<List<Datum[]>> expected = new ArrayList<List<Datum[]>>();
    expected.add(getResults0());
    TestHarness.testProgram(new File("test/CONST06.SQL"),
                            expected);
    System.out.println("Passed SQL Test CONST06");
  }
  ArrayList<Datum[]> getResults0() {
    ArrayList<Datum[]> ret = new ArrayList<Datum[]>();
    ret.add(new Datum[] {new Datum.Flt(1.2E-4)}); 
    return ret;
  }
}
